...
describe('Adding Changes', function () {
    it('adds a change to history', function () {
        $node = new Variable('x');
        $change = new Change(
            Change::TYPE_RENAME,
            'Renamed variable',
    ...
    it('adds multiple changes', function () {
        for ($i = 1; $i <= 5; $i++) {
...
describe('Undo/Redo', function () {
    ...
    it('returns null when nothing to undo', function () {
        $this->history->undo();
        $this->history->undo();
        $this->history->undo();
    ...
    it('can redo multiple changes', function () {
        $this->history->undo();
        $this->history->undo();
        $this->history->undo();
    ...
    it('returns null when nothing to redo', function () {
...
describe('History Navigation', function () {
    ...
    it('clears future history on new change after undo', function () {
        $this->history->undo();
        $this->history->undo();
...
describe('History Management', function () {
    ...
    it('returns all changes', function () {
        for ($i = 1; $i <= 3; $i++) {
            $node = new Variable("x{$i}");
...
describe('Change Object', function () {
    it('creates change with all properties', function () {
        $node = new Variable('x');
        $timestamp = time();
    ...
    it('auto-generates timestamp', function () {
        $node = new Variable('x');
        $before = time();
    ...
    it('formats change correctly', function () {
        $node = new Variable('x');
        $change = new Change(
            Change::TYPE_RENAME,
            'Renamed variable',
