...
it('marks pure nodes with PURE effect type', function () {
    $code = '<?php $x = 1 + 2;';
...
it('marks echo nodes with IO effect type', function () {
    $code = '<?php echo "Hello";';
    $ast = $this->parser->parse($code);
    $ast = $this->traverser->traverse($ast);
...
it('marks $_GET access with EXTERNAL_STATE effect type', function () {
...
it('marks rand() with NON_DETERMINISTIC effect type', function () {
...
it('marks file operations with IO effect type', function () {
...
it('collects statistics about effect types', function () {
    $code = '<?php
        $x = 1 + 2;          // PURE
        echo "test";         // IO
...
it('calculates percentage statistics correctly', function () {
    $code = '<?php
        $x = 1;
        $y = 2;
        echo "test";
...
it('resets statistics correctly', function () {
    $code = '<?php $x = 1;';
    $ast = $this->parser->parse($code);
    $ast = $this->traverser->traverse($ast);
...
it('finds nodes by effect type', function () {
    $code = '<?php
        $x = 1 + 2;
        echo "test";
        $y = 3 + 4;
...
it('handles mixed code with multiple effect types', function () {
    $code = '<?php
        function test() {
            $a = 1 + 2;
            echo $a;
...
it('marks all nodes in the tree', function () {
    $code = '<?php
        if ($x > 0) {
            echo "positive";
        } else {
...
it('handles nested expressions correctly', function () {
...
it('marks database operations with DATABASE effect type', function () {
...
it('marks curl operations with HTTP effect type', function () {
...
it('marks global state operations with GLOBAL_STATE effect type', function () {
...
it('returns empty array when finding nodes without matches', function () {
    $code = '<?php $x = 1;';
    $ast = $this->parser->parse($code);
    $ast = $this->traverser->traverse($ast);
