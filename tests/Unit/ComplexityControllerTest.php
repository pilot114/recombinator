...
use Recombinator\ComplexityController;
use Recombinator\ComplexityWarning;
...
test('registers before metrics', function () {
    $code = '<?php
        if ($x > 0) {
            echo "positive";
        }
...
test('registers after metrics and checks changes', function () {
    $code = '<?php
        if ($x > 0) {
            echo "positive";
        }
...
test('detects threshold exceeded for cognitive complexity', function () {
    $complexCode = '<?php
        function complex() {
            if ($a > 0) {
                if ($b > 0) {
...
test('detects threshold exceeded for cyclomatic complexity', function () {
    $complexCode = '<?php
        if ($a) {}
        if ($b) {}
        if ($c) {}
...
test('detects complexity increase', function () {
...
test('detects improvement', function () {
    $complexCode = '<?php
        if ($x > 0) {
            echo "x positive";
        }
...
test('strict mode treats increase as error', function () {
    $simpleCode = '<?php echo "test";';
    $complexCode = '<?php
        if ($x > 0) {
            echo "positive";
...
test('gets warnings by level', function () {
    $simpleCode = '<?php echo "test";';
    $complexCode = '<?php
        if ($x > 0) {
            echo "positive";
...
test('checks for errors', function () {
    $code = '<?php echo "test";';
    $ast = $this->parser->parse($code);
...
test('checks for warnings', function () {
    $simpleCode = '<?php echo "test";';
    $complexCode = '<?php
        if ($x > 0) {
            echo "positive";
...
test('gets comparison for identifier', function () {
    $code = '<?php echo "test";';
    $ast = $this->parser->parse($code);
...
test('returns null for non-existent comparison', function () {
    $comparison = $this->controller->getComparison('nonExistent');

    expect($comparison)->toBeNull();
});

test('clears all data', function () {
    $code = '<?php echo "test";';
    $ast = $this->parser->parse($code);
...
test('creates threshold exceeded warning', function () {
...
test('creates increased complexity warning', function () {
...
test('creates improvement info', function () {
...
test('gets warning context', function () {
    $warning = ComplexityWarning::thresholdExceeded('testFunc', 'cognitive', 20, 15);
...
test('handles multiple functions', function () {
    $simpleCode = '<?php echo "test";';
