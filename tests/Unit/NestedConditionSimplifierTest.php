...
it('simplifies nested if conditions', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            return true;
...
it('analyzes nesting complexity', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            if ($c) {
...
it('detects issues with deep nesting', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            if ($c) {
...
it('does not detect issues with shallow nesting', function () {
    $code = '<?php
    if ($a) {
        return true;
    }';
...
it('counts complex nodes correctly', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            if ($c) {
...
it('generates nesting report', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            return true;
...
it('handles code without nesting', function () {
    $code = '<?php
    $x = 1;
    $y = 2;
    $z = $x + $y;';
...
it('handles multiple if statements at same level', function () {
    $code = '<?php
    if ($a) {
        echo "a";
    }
...
it('analyzes average nesting', function () {
    $code = '<?php
    if ($a) {
        if ($b) {
            echo "nested";
...
it('does not modify simple code', function () {
    $code = '<?php
    if ($a) {
        return true;
    }';
    ...
    expect(strlen($simplifiedCode))->toBeLessThanOrEqual(strlen($originalCode) + 50);
});
