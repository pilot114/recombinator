...
it('finds related variables from same array', function () {
    $code = '<?php
    $x = $_GET["x"];
    $y = $_GET["y"];
    $z = $_GET["z"];';
...
it('finds related variables from same object', function () {
    $code = '<?php
    $x = $point->x;
    $y = $point->y;
    $z = $point->z;';
...
it('does not group unrelated variables', function () {
    $code = '<?php
    $x = $_GET["x"];
    $y = $_POST["y"];
    $z = $someVar;';
...
it('handles AST with no variable assignments', function () {
    $code = '<?php
    echo "Hello";
    echo "World";';
...
it('handles single variable assignment', function () {
    $code = '<?php
    $x = $_GET["x"];';
...
it('identifies variables from array access', function () {
    $code = '<?php
...
it('creates RelatedVariableGroup with correct size', function () {
    $assignments = [
...
it('generates unique signature for variable group', function () {
    $assignments1 = [
        ['var' => '$x', 'expr' => null, 'index' => 0],
        ['var' => '$y', 'expr' => null, 'index' => 1],
    ];
...
it('gets correct indices from variable group', function () {
    $assignments = [
...
it('handles complex expressions in variable assignments', function () {
    $code = '<?php
    $x = $data["coords"]["x"];
    $y = $data["coords"]["y"];';
...
it('preserves non-assignment statements', function () {
    $code = '<?php
    $x = $_GET["x"];
    echo "Processing...";
    $y = $_GET["y"];';
...
it('ignores dynamic variable assignments', function () {
    $code = '<?php
    ${$varName} = "value";
    $normal = "test";';
...
it('handles variables from different array keys', function () {
    $code = '<?php
    $a = $arr["first"];
    $b = $arr["second"];
    $c = $arr["third"];';
...
it('handles variables from different objects', function () {
    $code = '<?php
    $x = $point1->x;
    $y = $point2->y;';
...
it('deduplicates variable groups correctly', function () {
...
it('handles mixed assignment types', function () {
    $code = '<?php
    $x = $_GET["x"];
    $obj = new stdClass();
    $y = $_GET["y"];';
...
it('preserves statement order when no optimization is applied', function () {
    $code = '<?php
    $a = 1;
    $b = 2;
    $c = 3;';
