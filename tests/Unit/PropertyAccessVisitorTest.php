...
use Recombinator\Visitor\PropertyAccessVisitor;
...
it('can replace property access with variable', function () {
    $code = '<?php
$obj = new TestClass();
echo $obj->name;';

    // Manually set up instance data
    $this->store->setInstanceToScope('obj', 'TestClass', [
        'properties' => [
            'name' => 'var_name_123'
        ]
    ]);
...
it('should not replace property if instance not found', function () {
...
it('should not replace property if property not in instance', function () {
...
it('can handle multiple property accesses', function () {
    $code = '<?php
$user = new User();
echo $user->name . " " . $user->age;';

    $this->store->setInstanceToScope('user', 'User', [
        'properties' => [
            'name' => 'var_name',
            'age' => 'var_age'
        ]
...
it('should preserve this property access for later processing', function () {
    $code = '<?php
class Test {
...
it('can find class name and instance from store', function () {
    $this->store->setInstanceToScope('myObj', 'MyClass', [
...
it('returns null when instance not found in store', function () {
    $result = $this->store->findClassNameAndInstance('nonExistent');

    expect($result)->toBeNull();
});

it('can replace nested property access', function () {
    $code = '<?php
$calc = new Calculator();
$result = $calc->value + 10;';

    $this->store->setInstanceToScope('calc', 'Calculator', [
        'properties' => [
            'value' => 'var_value'
        ]
    ]);
